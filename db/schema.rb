# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# This file is the source Rails uses to define your schema when running `bin/rails
# db:schema:load`. When creating a new database, `bin/rails db:schema:load` tends to
# be faster and is potentially less error prone than running all of your
# migrations from scratch. Old migrations may fail to apply correctly if those
# migrations use external dependencies or application code.
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema.define(version: 2021_09_21_014347) do

  create_table "animal_habitats", force: :cascade do |t|
    t.integer "animal_id", null: false
    t.integer "habitat_id", null: false
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
    t.index ["animal_id"], name: "index_animal_habitats_on_animal_id"
    t.index ["habitat_id"], name: "index_animal_habitats_on_habitat_id"
  end

  create_table "animal_types", force: :cascade do |t|
    t.integer "animal_id", null: false
    t.integer "type_id", null: false
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
    t.index ["animal_id"], name: "index_animal_types_on_animal_id"
    t.index ["type_id"], name: "index_animal_types_on_type_id"
  end

  create_table "animals", force: :cascade do |t|
    t.string "name", null: false
    t.text "descripcion", null: false
    t.boolean "extinto", default: false
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
    t.decimal "tamaño"
    t.boolean "sexualidad"
    t.decimal "genero"
    t.string "foto"
    t.string "nombreCien"
  end

  create_table "habitats", force: :cascade do |t|
    t.string "name", null: false
    t.text "descripcion"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
  end

  create_table "types", force: :cascade do |t|
    t.string "name", null: false
    t.text "descripcion"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
  end

end
